...
MinecraftForge 26.2 (Forge 65.x) port of SoulSort. Same idea as the Fabric build:
...
Needs JDK 25 (Minecraft 26.2's requirement). `gradlew build`; the build script pulls
a matching JDK via Gradle's toolchain support if needed. Output jar goes in the
server or client `mods/` folder.
