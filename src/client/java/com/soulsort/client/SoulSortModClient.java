package com.soulsort.client;

import com.soulsort.SoulSortMod;
 ...
 * The detection channel is registered by SoulSortMod (which runs here too), so
 * there's nothing to register here - doing so would double-register and crash.
 */
public class SoulSortModClient implements ClientModInitializer {
	@Override
	public void onInitializeClient() {
		ClientPlayConnectionEvents.JOIN.register((handler, sender, client) -> {
			if (client.isLocalServer() || ClientPlayNetworking.canSend(SoulSortMod.HELLO_ID)) {
				return;
			}
			client.execute(() -> {
				if (client.player != null) {
					client.player.sendSystemMessage(Component.literal(warning(client)));
				}
			});
		});
	}
